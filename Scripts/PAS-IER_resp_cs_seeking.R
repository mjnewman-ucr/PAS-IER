...
library(pacman)
p_load(tidyverse, dplyr, readr, lubridate, Hmisc, DataExplorer, ggpubr, moments, 
       scales, sjmisc, pequod, stargazer, effects, emmeans, lsr, rstatix, vcov, 
       apaTables, reghelper, car)
...
#no longer need to remove the one participant who has a shit ton of missing data,
#because I included that in my data cleaning code
...
ds_cor <- select(ds, c(iris_r, iris_cs_centered, seeking_unfactored, ppass_as, eff_comp))
...
#This lil section below runs the correlations with simplified data
...
# apa.cor.table(
#  ds_cor_demo,
#  filename = "main_correlations_demos.doc",
#  landscape = T)

#-------------------------------------------------------------------------------------------------------------

#Scatterplots of IER processes with IER effectiveness
plot(ds_cor$iris_cs_centered, ds_cor$eff_comp, main="Scatterplot",
     xlab="IER cognitive support", ylab="IER effectiveness", pch=20)
...
ds %>%
  group_by(seeking) %>%
  get_summary_stats(ppass_as, type = "mean_sd")
...
#Controlling for IER cognitive support in the modeL
...
#   process_rcs_model1,
#   process_rcs_model2,
#   filename = "ier_responsiveness_with_cs.doc")
...
#Controlling for IER repsonsiveness in the modeL
...
#   process_csr_model1,
#   process_csr_model2,
#   filename = "ier_cogsupport_with_r.doc")
...
#----------------------------------------------------------------------------------------------------
#Exploring possible parent gender differences
...
#----------------------------------------------------------------------------------------------------
#Exploring possible home differences
